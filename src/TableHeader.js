...
function TableHeader({patientDetails, verifiedKeys, handleVerification, listening}) {
    ...
    return (
        <div style={{padding: '2px', borderRadius: '20px'}}>
            <Title level={4} style={{color: '#2952EF', marginTop: '0px'}}>{listening ? `Patient Info - ${patientDetails["patient_id"] ? patientDetails["patient_id"] : 'Loading..'}` : 'Start recording to load the data'}</Title>
            <Row gutter={[16, 16]}>
